-- Profile statistics RPC (Decisions.md B2–B4)
...
BEGIN
  ...
  -- Consistent since: consecutive calendar days with done check-ins (global, excludes missed)
  WITH done_days AS (
  ...
  -- Longest streak: max consecutive done days within any single challenge
  FOR v_challenge IN
  ...
  RETURN jsonb_build_object(
    ...
    'wager_settlement_ratio',
      CASE WHEN v_wagers_total > 0
        THEN ROUND((v_wagers_settled::numeric / v_wagers_total) * 100)
        ELSE 100 END,
